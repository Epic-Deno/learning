import React, { Component } from 'react';
import {
    View,
    ImageBackground,
    Image,
    StyleSheet,
    Text
} from "react-native"
export default class LkImage extends Component {
    constructor() {
        super();
        this.state = {

        }
    }
    render() {

        return (
            <View>
                {/* 加载本地图片 */}
                {/* <Image source={require('../assets/images/calculate.png')} style={{ width: 200, height: 200 }} /> */}
                {/* 加载base64或者网络图片 */}
                {/* <Image source={{ uri: 'data:image/svg+xml;base64,' }} style={{ width: 60, height: 20 }} /> */}
                
                {/* 图片作为背景 */}
                {/* <ImageBackground source={require('../assets/images/calculate.png')} style={{width:300,height:300}}>
                    <Text style={{color:'#0F0'}}>
                        逍遥子老哥
                    </Text>
                    <Image source={require('../assets/images/React.png')} style={{width:80,height:80}}/>
                </ImageBackground> */}
                {/* 图片作为背景图 */}
                <View style={{backgroundColor:'#0f0'}}>
                    <Image source={require('../assets/images/React.png')} style={{width:400,height:400,resizeMode:'center'}}
                    />
                </View>
            </View>
        )
    }
}
const Styles = StyleSheet.create({



});